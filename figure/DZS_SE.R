...
ggplot(
  ...
  geom_errorbar(
    data = filter(plot.data2n, Type == "Unconjugated BAs"),
    aes(
      ymin = newMean - mySE,
      ymax = newMean + mySE
    ),
...
ggplot(
  ...
  geom_text(
    aes(Group, myMEAN + mySE + 0.05 * top,
      label = s
    ),
    size = 10
  ) +
