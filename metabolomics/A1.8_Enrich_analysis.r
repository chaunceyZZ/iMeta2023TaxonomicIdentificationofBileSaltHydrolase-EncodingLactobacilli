...
#dataFile <- "total.enriched.gene.nred.2.dat";
#outDir <- "./";
...
for(i in 1:length(Groups))
{
	...
	write.csv(summary(ekk),paste(outDir,"/",SamID,".",Groups[i],".KEGGenrich.csv",sep=""),row.names=F);
}
...
pdf(paste(outDir,"/",SamID,".enriched.dot.pdf",sep=""),width=8,height=5)
plot(gene.comp);
