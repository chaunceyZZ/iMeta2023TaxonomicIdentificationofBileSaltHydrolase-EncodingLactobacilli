...
{
        groupname <- append(groupname,as.character(grpInfo[grep(colnames(Abundance)[i],as.character(grpInfo[,1])),grp_col]));
}
...
write.table(Max.Group,file=paste(outDir,"/",SamID,".enriched.metabolite.dat",sep=""),quote = F,sep="\t",col.names=NA);
