...
## docker images: chaunceyzhou/cpu-qiime-191
arg <- commandArgs(T)
if(length(arg) != 4){
...
Alpha.Group.mean <- unlist(Alpha.Group.mean)
Alpha.Group.sd <- unlist(Alpha.Group.sd)
...
bar.pos <- barplot(
  ...
  names.arg="",
  ylim=c(0,y.lim),
  ylab=paste(MetID,"Index",sep=" "),
  cex.names=1.0, cex.lab=1.0, cex=1.2
)
