...
print(arg)
#arg <- c( "/home/Project/16S/LSZ_20190908/Analysis/9_PICRUSt/Total/Total.catagrized.cog.L2.tab","/home/Project/16S/LSZ_20190908/bin/Total-color.txt" , "/home/Project/16S/LSZ_20190908/Analysis/9_PICRUSt/Total" ,"2"  )
dataFile <- arg[1];
groupFile <- arg[2];
outDir <- arg[3];
...
#Debug
#dataFile <- "gut.catagrized.cog.L2.tab";
#groupFile <- "gut-grouping.info";
#outDir <- "/home/Metagenome-2/Asprin-Run1-16S/Analysis/9_PICRUSt";
#grp_col <- 2;

SamID <- strsplit(basename(dataFile),'.',fixed=T)[[1]][1];
FunID <- strsplit(basename(dataFile),'.',fixed=T)[[1]][3];

data <- read.table(dataFile,header=T,sep = "\t",row.names=1,check.names = FALSE);
data <- data[,-ncol(data)];
myFun=function(x){x/sum(x)};
Abundance <- data.frame(apply(data,2,myFun),check.names = FALSE)
...
ii <- intersect(rownames(grpInfo),colnames(Abundance))
...
grpInfo=grpInfo[ii,]
Abundance=Abundance[,ii]




write.table(round(Abundance,6),file=paste(outDir,"/",SamID,".",FunID,".L3.abundance.txt",sep=""),quote = F,sep="\t",col.names=NA);
...
pdf(paste(outDir,"/",SamID,".",FunID,".L3.abundance.boxplot.pdf",sep=""),width=12,height=7.5);
...
par(mfrow=c(1,1),mar=c(20,5,5,5));
boxplot(Top.abundance.perc[1:top_top_n,]~Top.group[1:top_top_n,],col=G_cc[Groups],xaxt="n",xlim=c(1,max(Top.group)),outline=F,ylab='',cex.axis=1.3);
text(x=seq((num_groups-1),(num_groups*top-1),num_groups),y=4,labels=Top.name,xpd=T,srt=45,pos=2,font=2,cex=0.9,offset=0)
...
boxplot(Top.abundance.perc[(top_top_n+1):nrow(Top.abundance.perc),]~Top.group[(top_top_n+1):nrow(Top.abundance.perc),],col=G_cc[Groups],xaxt="n",xlim=c(1,max(Top.group)),at=min(Top.group[(top_top_n+1):nrow(Top.abundance.perc),]):max(Top.group[(top_top_n+1):nrow(Top.abundance.perc),]),yaxt='n',outline=F,ylab='');
...
boxplot(Top.abundance.perc[(top_top_n+1):nrow(Top.abundance.perc),]~Top.group[(top_top_n+1):nrow(Top.abundance.perc),],col=G_cc[Groups],xaxt="n",xlim=c(1,max(Top.group)),at=min(Top.group[(top_top_n+1):nrow(Top.abundance.perc),]):max(Top.group[(top_top_n+1):nrow(Top.abundance.perc),]),add=T,xaxt='n',yaxt='n',outline=F);
